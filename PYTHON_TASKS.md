...
Интерпретатор Python после установки располагается, обычно, по пути /usr/local/bin/python3.1 или /usr/local/bin/python2.7. Для проверки наличия интерпритатора в системе можно воспользоваться системной утилитами `whereis` или `which` (это верно  относительно UNIX подобных систем).
...
Теперь вы можете работать непосредственно с интеретатором вводя команды. Чтобы завершить работу интерактивной оболочки Pyhton можно нажать клавиши <kbd>Ctrl</kbd> + <kbd>D</kbd>  <kbd>Ctrl</kbd> + <kbd>C</kbd>.
