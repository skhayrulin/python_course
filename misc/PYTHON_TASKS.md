...
Интерпретатор Python после установки располагается, обычно, по пути /usr/local/bin/python3.6 или /usr/local/bin/python2.7. Для проверки наличия интерпритатора в системе можно воспользоваться системной утилитами `whereis` или `which` (это верно  относительно UNIX систем).
...
Теперь вы можете работать непосредственно с интеретатором вводя команды. Чтобы завершить работу интерактивной оболочки Pyhton можно нажать клавиши <kbd>Ctrl</kbd> + <kbd>D</kbd>, также можно воспользоваться командой `quit()` или `exit()`.
...
1. Вычислите степень 0.5 из отрицательного числа
...
Тип переменной в python определяется динамически, информация о типе хранится в значении переменной, так, например будет корректен следующий код
...
Как вы видели имя переменной не менялось и более того вы можете поверить, что это одна и тоже переменная, меняется только ее значение. Итак, переменная задается следующим образом `var_name = value`. Переменная создается при ее инициализации. Можно комбинировать различные арифметические операции и присваивание например:
    
    x += 10  # это эквивалентно x = x + 10
    x *= 10  # это эквивалентно x = x * 10
    x **= 10 # это эквивалентно x = x**10

Для удаления (очистки памяти занимаемой переменной) переменной и используйте функцию del.
...
Любопытная особенность питона: можно использовать привычные из математики сравнения вроде `x < y` and `y < z`.
...
2. Опишите несколько из объектов перечисленых выше на Python.
3. Напишите структуры классов для некоторой древовидной структуры данных: дерево, корень, потомки. 
4. Реализуйте метод прохода по такй структуре (в глубину, в ширину)
...
### Задачи
1. Для вашей структы данных придумайте, несколько типов вершин дереава.
2. каждая вершина, может содержа в качестве потомков разныхз типов. Выделете для вершин дерева общий интерфейс.
...
### Задачи
1. Для вашей структы данных придумайте, какие поля можно скрыть, напишите методы, которые бы позволяли обращаться к этим полям на чтение.
...
Предлагается два варианта финального задания (необходимо выполнить один из вариантов)

## Калькулятор вариант 1

Написать калькулятор исполняющий команды арифметических операций используя классы и стандартные библиотеки python. Калькулятор должен уметь исполнять команды, выводить ошибки о неверных значениях операндов, необъявленных переменных, неизвестных командах итп.
...
Реализовать работу с переменными (перед работой с переменной в калькуляторе её обязательно нужно объявлять, если переменная не объявлена, то калькулятор должен сообщить об этом в строке вывода)!

* ```SET var_name var_value``` --- ```var_name = var_value``` - объявление переменной и присваивание ей определенного значения
* ```PRINT var_name``` --- ```print(var_name)``` - вывод значения перменной
* в базовом наборе команд могут использоваться переменные, причем
  * если переменная в правом операнде, то используется её значение:
  ```
  >>> SET x 5
  >>> ADD 2 x   // ADD 2 5
  7
...
Реализовать работу с функциями.
...
**ФУНКЦИЕЙ EVAL ПОЛЬЗОВАТЬСЯ ЗАПРЕЩАЕТСЯ.**
